...
//     (\`new ApiErrorHandlerSDK(...)\` / \`ApiErrorHandlerSDK.test(...)\` become
//     \`ApiErrorHandlerSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ApiErrorHandlerSDK'
...
const TEST_SEED = {"entity":{"logo_generation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ApiErrorHandlerSDK(...)\` and any
// \`ApiErrorHandlerSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ApiErrorHandlerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ApiErrorHandlerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
