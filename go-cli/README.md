# api-error-handler-cli

boru-driven command-line client **and** interactive REPL for the ApiErrorHandler
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api-error-handler-cli)
make build

# 2. See usage (words, entities, env vars)
./api-error-handler-cli --help

# 3. Provide credentials once, via the environment
export API_ERROR_HANDLER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./api-error-handler-cli load 1 logo_generation            # {id:1} shorthand
./api-error-handler-cli load '{id:1}' logo_generation       # explicit match map

# 5. Override the API base URL for a single call
API_ERROR_HANDLER_BASE=https://api.example.com ./api-error-handler-cli load 1 logo_generation

# 6. No arguments -> interactive REPL
./api-error-handler-cli
api-error-handler> load 1 logo_generation
api-error-handler> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api-error-handler-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/api-error-handler-cli load 1 logo_generation
   ```
...
```sh
./api-error-handler-cli load 1 logo_generation          # scalar shorthand for {id:1}
./api-error-handler-cli load '{id:1}' logo_generation     # explicit match map
```
...
./api-error-handler-cli load 1 logo_generation
```
...
Run with no arguments to open a REPL (prompt `api-error-handler>`). Each line is
evaluated as its own boru expression:

```text
$ ./api-error-handler-cli
api-error-handler> load 1 logo_generation
api-error-handler> /help
api-error-handler> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `logo_generation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/api-error-handler-cli`. |
...
(`./api-error-handler-cli <expr>`) and interactively (the REPL), and expressions compose the
