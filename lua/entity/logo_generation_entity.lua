...
local LogoGenerationEntity = {}
LogoGenerationEntity.__index = LogoGenerationEntity


function LogoGenerationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LogoGenerationEntity)
  self._name = "logo_generation"
  self._client = client
...
function LogoGenerationEntity:get_name()
  return self._name
end


function LogoGenerationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LogoGenerationEntity.new(self._client, opts)
end
...
function LogoGenerationEntity:mark_deleted()
  self._deleted = true
end


function LogoGenerationEntity:deleted()
  return true == self._deleted
end


function LogoGenerationEntity:data_set(args)
  if args ~= nil then
...
function LogoGenerationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LogoGenerationEntity:match_set(args)
  if args ~= nil then
...
function LogoGenerationEntity:match_get()
...
function LogoGenerationEntity:stream(action, args, callopts)
...
---@param reqmatch LogoGenerationLoadMatch
---@param ctrl? table
---@return LogoGeneration
---@return string? err
function LogoGenerationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LogoGenerationEntity:_run_op(ctx, post_done)
  local utility = self._utility
