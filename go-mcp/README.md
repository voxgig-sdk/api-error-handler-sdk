# api-error-handler-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ApiErrorHandler SDK as
two agent tools — `api-error-handler_list` and `api-error-handler_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api-error-handler-mcp)
make build

# 2. Provide credentials via the environment
export API_ERROR_HANDLER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user api-error-handler \
  -- /absolute/path/to/api-error-handler-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./api-error-handler-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// api-error-handler_list: first page of records
{ "entity": "logo_generation" }
{ "entity": "logo_generation", "query": { } }

// api-error-handler_load: one record by id
{ "entity": "logo_generation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api-error-handler-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user api-error-handler \
     -- "$PWD"/dist/*/api-error-handler-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `api-error-handler_list` and `api-error-handler_load` tools now appear
   in new sessions. Ask the agent to *"list logo_generation using api-error-handler"*
   and it calls `api-error-handler_list` with `{"entity":"logo_generation"}`.
...
```sh
export API_ERROR_HANDLER_APIKEY=sk_live_xxx            # API key
export API_ERROR_HANDLER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `api-error-handler_list` tool
...
### Call the `api-error-handler_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `api-error-handler_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `api-error-handler_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `API_ERROR_HANDLER_APIKEY` | API key sent with every request. |
| `API_ERROR_HANDLER_BASE` | Optional override of the API base URL. |
...
```sh
./api-error-handler-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"api-error-handler_load","arguments":{"entity":"logo_generation","query":{"id":1}}}}'
```
