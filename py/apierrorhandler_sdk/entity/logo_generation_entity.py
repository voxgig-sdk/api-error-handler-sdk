...
from apierrorhandler_sdk.apierrorhandler_types import (
    LogoGeneration,
...
class LogoGenerationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LogoGenerationEntity(self._client, opts)
    ...
    def data_get(self) -> LogoGeneration:
    ...
    def match_get(self) -> LogoGeneration:
    ...
    def load(self, reqmatch=None, ctrl=None) -> LogoGeneration:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.LogoGeneration().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
