# ApiErrorHandler SDK test runner
...
class ApiErrorHandlerTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ApiErrorHandlerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ApiErrorHandlerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ApiErrorHandlerTestRunner.getenv("API_ERROR_HANDLER_TEST_LIVE")
        override = ApiErrorHandlerTestRunner.getenv("API_ERROR_HANDLER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ApiErrorHandlerTestRunner.getenv(key)
        ...
        explain = ApiErrorHandlerTestRunner.getenv("API_ERROR_HANDLER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ApiErrorHandlerTestRunner._test_control is not None:
            return ApiErrorHandlerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ApiErrorHandlerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ApiErrorHandlerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ApiErrorHandlerTestRunner.load_test_control()
...
def load_env_local():
    ApiErrorHandlerTestRunner.load_env_local()


def env_override(m):
    return ApiErrorHandlerTestRunner.env_override(m)


def entity_data(v):
    return ApiErrorHandlerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ApiErrorHandlerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ApiErrorHandlerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ApiErrorHandlerTestRunner.load_test_control()


def live_client_options():
    return ApiErrorHandlerTestRunner.live_client_options()


def live_delay_ms():
    return ApiErrorHandlerTestRunner.live_delay_ms()
