...
This API is designed to manage and provide standardized responses for different error scenarios encountered during API calls.

Learn more about API Error Handler at [abhi-api.vercel.app](https://abhi-api.vercel.app).
...
// logogeneration is the LogoGeneration entity, populated with mock data
...
// Load a specific logogeneration (returns the ENTITY; call data_get() for the record; throws on error)
$logogeneration = $client->LogoGeneration()->load(["text" => "example_text"]);
...
# Load a specific logogeneration (returns the ENTITY; call data_get for the record)
logogeneration = client.LogoGeneration.load({ "text" => "example_text" })
puts logogeneration
```
