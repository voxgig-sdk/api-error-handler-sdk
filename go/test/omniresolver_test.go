...
import (
	...
	omni "github.com/voxgig-sdk/api-error-handler-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ApiErrorHandlerError); is {
				out := map[string]any{
					"name":    "ApiErrorHandlerError",
...
func omniCtx(arg any, client *sdk.ApiErrorHandlerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ApiErrorHandlerSDK); is {
			client = live
