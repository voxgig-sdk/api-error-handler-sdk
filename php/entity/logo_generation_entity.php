...
// ApiErrorHandler SDK LogoGeneration entity
...
class LogoGenerationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LogoGenerationEntity($this->_client, $opts);
    }

    /**
     * @param LogoGeneration|array $args LogoGeneration data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ApiErrorHandlerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return LogoGeneration|array The current LogoGeneration data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of LogoGeneration fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ApiErrorHandlerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of LogoGeneration fields).
     */
    ...
    /**
     * Load a single LogoGeneration.
     *
     * @param LogoGenerationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed LogoGenerationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return LogoGeneration|array The loaded LogoGeneration as an assoc-array at the
     *   SDK boundary; throws ApiErrorHandlerError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ApiErrorHandlerHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
