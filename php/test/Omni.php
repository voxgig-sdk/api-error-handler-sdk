...
//    only - a typed ApiErrorHandlerContext there would make every ctx
...
final class ApiErrorHandlerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ApiErrorHandlerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ApiErrorHandlerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ApiErrorHandlerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ApiErrorHandlerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ApiErrorHandlerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ApiErrorHandlerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ApiErrorHandlerError(is_string($code) ? $code : '', $msg);
    }
