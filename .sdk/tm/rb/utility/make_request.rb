...
module ApiErrorHandlerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ApiErrorHandlerResponse.new({})
    ...
    if fetch_err
      ...
      response = ApiErrorHandlerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ApiErrorHandlerResponse.new(fetched)
    else
