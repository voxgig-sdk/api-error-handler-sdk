...
// ApiErrorHandler SDK feature test
//
...
class FtCtrl extends ApiErrorHandlerControl
{
...
class FtClient
{
    ...
    private ApiErrorHandlerUtility $utility;

    public function __construct(ApiErrorHandlerUtility $utility, array $options)
    {
    ...
    public function get_utility(): ApiErrorHandlerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ApiErrorHandlerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ApiErrorHandlerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ApiErrorHandlerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ApiErrorHandlerConfig::make_feature($name);
    ...
    private static function buildUrl(ApiErrorHandlerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ApiErrorHandlerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ApiErrorHandlerResponse($fetched);
        }
    ...
    private function fail(ApiErrorHandlerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ApiErrorHandlerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
