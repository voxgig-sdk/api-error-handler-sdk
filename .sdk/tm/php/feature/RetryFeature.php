...
// ApiErrorHandler SDK retry feature
...
class ApiErrorHandlerRetryFeature extends ApiErrorHandlerBaseFeature
{
    ...
    public function init(ApiErrorHandlerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ApiErrorHandlerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ApiErrorHandlerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ApiErrorHandlerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
