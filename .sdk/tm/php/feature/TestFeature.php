...
// ApiErrorHandler SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ApiErrorHandlerTestFeature extends ApiErrorHandlerBaseFeature
{
    ...
    public function init(ApiErrorHandlerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ApiErrorHandlerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ApiErrorHandlerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ApiErrorHandlerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ApiErrorHandlerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ApiErrorHandlerParam::call($ctx, $k);
