...
require_relative 'ApiErrorHandler_types'
...
class ApiErrorHandlerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ApiErrorHandlerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ApiErrorHandlerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ApiErrorHandlerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ApiErrorHandlerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ApiErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ApiErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ApiErrorHandlerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ApiErrorHandlerError => err
    ...
    if fetched.is_a?(Hash)
      status = ApiErrorHandlerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ApiErrorHandlerError.new(
        "graphql_error", "ApiErrorHandlerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ApiErrorHandlerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
