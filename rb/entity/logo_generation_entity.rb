...
class LogoGenerationEntity
  ...
  def make
    opts = @_entopts.dup
    LogoGenerationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ApiErrorHandlerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [LogoGeneration, Hash] the current LogoGeneration data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ApiErrorHandlerHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of LogoGeneration fields)
  def match_get
  ...
  #
  # @param reqmatch [LogoGenerationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.LogoGeneration.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [LogoGeneration, Hash] the loaded LogoGeneration; raises ApiErrorHandlerError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ApiErrorHandlerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
